...
function App() {
  ...
  const [newTodo, setNewTodo] = useState<Todo>({ name: '', description: '', isDone: false })
  const [name, setName] = useState("");
  const [description, setDescription] = useState("");

  const addTodo = () =>
    setTodoList((p) => [...p, { name, description, isDone: false }]);
    ...
    <div style={{ width: "100%" }}>
      <div style={{ display: "flex", gap: "10px" }}>
        <input
          style={{ width: "100%" }}
          value={name}
          onChange={(event) => setNewTodo(event, setName)}
        />
        <input
          style={{ width: "100%" }}
          value={description}
